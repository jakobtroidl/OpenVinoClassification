...
def main():
    ...
    # if true folder is processed, else single file or live video is processed
    process_folder = os.path.isdir(input_stream)
    process_video = False

    if process_folder:
        onlyfiles = [f for f in listdir(input_stream) if isfile(join(input_stream, f))]
    else:
        if input_stream != 0 and input_stream.endswith(".mp4"):
            process_video = True
    ...
    is_async_mode = True
    if process_folder:
        number_input_frames = len(onlyfiles)
    else:
    ...
    if number_input_frames != 1 and not process_folder:
        ret, frame = cap.read()
    else:
        is_async_mode = False
        wait_key_code = 0
    ...
    while True:
        ...
        # exit if all frames are processed

        if process_folder and frames_counter == len(onlyfiles):
            break

        if is_async_mode:

            if process_folder:
                file_path = input_stream + onlyfiles[frames_counter]
                print("Path: ", file_path)
                next_frame = cv2.imread(file_path)
            else:
        ...
        if not process_folder and not ret:
            break
        ...
        if process_folder:
            frame_header = str(onlyfiles[frames_counter])
        else:
            frame_header = str(frames_counter)

        print(frame_header, ": ", len(objects))
        ...
        if process_video:
            ...
            if process_folder:
                output_filename = result_path + onlyfiles[frames_counter]
            else:
    ...
    if process_video:
        out.release()
    print("Release outputstream called")
